...
def get_parser():
    ...
    parser.add_argument('--fp16', default=False, action='store_true', help='Run model fp16 mode. Default True')
...
log.console("using seed "+str(args.seed))
...
def main():
    ...
    if args.sparsification:
        model = RandomKSparsifiedDDP(model, device_ids=[args.local_rank], output_device=args.local_rank, randk=args.randk)
    ...
    if args.sparsification:
        args.epsilon = [torch.zeros(layer.numel(), dtype=layer.dtype).cuda() for layer in model.parameters()]
